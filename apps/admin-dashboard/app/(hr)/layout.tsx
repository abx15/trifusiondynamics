...
export default function HrLayout({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    async function restoreSession() {
      ...
      if (currentUser) {
        ...
        if (primary !== "hr_agent" && primary !== "admin" && primary !== "super_admin") {
  ...
  const navItems = [
    ...
    { label: "Recruitment", href: "/hr/recruitment", icon: ClipboardList },
    { label: "Security & Password", href: "/hr/settings", icon: Settings },
  ];
      ...
      <aside className="w-64 border-r border-zinc-800 bg-zinc-900/60 flex flex-col justify-between p-4 shrink-0">
        <div>
          ...
          <div className="flex items-center gap-3 px-3 py-4 mb-6 border-b border-zinc-800">
            <div className="h-10 w-10 rounded-xl bg-white flex items-center justify-center shadow-lg shadow-emerald-600/20 ring-2 ring-emerald-500/30 overflow-hidden shrink-0">
              <Image src="/logo.png" alt="Trifusion Dynamics" width={36} height={36} className="object-contain" />
            </div>
            <div>
              <h1 className="font-bold text-sm tracking-tight text-white">HR Portal</h1>
              <div className="inline-flex items-center gap-1 text-[10px] uppercase font-semibold text-emerald-400">
                <HardHat className="w-3 h-3" /> People Hub
              </div>
            </div>
          </div>
        ...
        <div className="pt-4 border-t border-zinc-800">
          <div className="px-3 py-2.5 rounded-xl bg-zinc-800/40 border border-zinc-800 mb-3 flex items-center gap-3">
            <div className="h-8 w-8 rounded-full bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 flex items-center justify-center font-bold text-xs">
              {user?.name ? user.name.charAt(0) : "H"}
            </div>
            <div className="overflow-hidden">
              <p className="text-xs font-semibold text-white truncate">{user?.name || "HR Agent"}</p>
              <p className="text-[10px] text-zinc-400 truncate">{user?.email || "hr@trifusion.com"}</p>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 border-b border-zinc-800 bg-zinc-900/40 px-6 flex items-center justify-between">
          <span className="text-sm font-semibold text-zinc-300">
            HR Dashboard — People Operations
          </span>
          <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold">
            <HardHat className="w-3.5 h-3.5" /> HR Access
          </div>
        </header>
